...
> Rules for AI coding agents using `bloom` (v1.6.0) to scaffold full-stack
...
4. For `userapp`, run `npx prisma db push` + edit `.env` before
   `npm run dev`.
5. Pin appkit + uikit to the `^1.5.1` range that the templates ship
   with. Don't change them unless you're tracking a coordinated major.
...
5. Never pin appkit or uikit to `latest`. The templates pin to
   `^1.5.1` for a reason — breaking changes in the ecosystem need a
   coordinated bloom release.
