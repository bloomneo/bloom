...
function asAppKitErrorish(error: unknown): AppKitErrorish | null {
  ...
  // Duck-type — `AppKitError` subclasses that survived the import boundary
  // but lost their prototype identity.
  ...
  // Message-prefix fallback — appkit modules that throw plain Errors but
  // self-format their messages. Extract module + docs URL from the text.
...
function formatRouteLoadError(
  ...
  if (appkit) {
    // appkit messages already contain `[module] text. See: docsUrl`; strip
    // the trailing `See:` line so we can render docs on its own line and
