...
export const config = {
  // API Configuration
  api: {
    // Desktop app: Use Electron's API or fallback to localhost
    baseUrl: (window as any).electronAPI?.apiBaseUrl || import.meta.env.VITE_API_URL || 'http://localhost:3000',
    endpoints: {
  ...
  auth: {
    ...
    redirects: {
      ...
      afterLogin: import.meta.env.VITE_AFTER_LOGIN || '/user',
      ...
      authenticatedRedirect: import.meta.env.VITE_AUTHENTICATED_REDIRECT || '/user'
    }
  },
